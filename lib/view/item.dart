...
/// ```
///
typedef Widget ItemWidgetBuilder(BuildContext context, double stuckAmount);
...
///
class Item extends MultiChildRenderObjectWidget {
  /// Constructs a new [item] widget.
  Item({
    Key? key,
  ...
  /// consider using [itemBuilder] instead.
  final RenderItemCallback? callback;

  @override
  RenderItem createRenderObject(BuildContext context) {
  ...
  @override
  void updateRenderObject(BuildContext context, RenderItem renderObject) {
    final scrollPosition = this.controller?.position ?? Scrollable.of(context)!.position;
    renderObject
      ..scrollPosition = scrollPosition
      ..callback = this.callback
...
///
/// The same as [item] but instead of supplying a Header view, you supply a [builder] that
/// constructs the header with the appropriate stickyness.
///
/// Place this widget inside a [ListView], [GridView], [CustomScrollView], [SingleChildScrollView] or similar.
///
class ItemBuilder extends StatefulWidget {
  /// Constructs a new [itemBuilder] widget.
  const ItemBuilder({
    Key? key,
